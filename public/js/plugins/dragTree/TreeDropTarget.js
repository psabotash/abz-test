...
TreeDropTarget.prototype._getTargetElem = function(avatar, event) {
  ...
  while (elem) {
    ...
    //if(elemToMove.className != "dropTarget") return;
...
TreeDropTarget.prototype.onDragEnd = function(avatar, event) {
  ...
  if (!this._targetElem || this._targetElem.parentNode.className != "dropTarget") {
  ...
  apply_changes(avatarInfo.dragZoneElem.parentNode.id, this._targetElem.parentNode.parentNode.parentNode.id);
